...
export async function runAdminSealedCardMarketPriceCheck(id: string) {
  ...
  const resolvedUrl = resolveSealedCardMarketExactSourceUrl({ cardmarketId: product.cardmarket_id, cardmarketUrl: product.cardmarket_url });
  if (!resolvedUrl) throw new AdminCardMarketPriceCheckError("This sealed product has no direct CardMarket product linked yet.", 409);
  ...
  if (!marketOffer) throw new AdminCardMarketPriceCheckError("No English sealed offers with a verified seller could be read. No prices were saved.", 422);
  const euOffer = sellerOffers.find((offer) => offer.isEu) ?? null;
  const observedAt = new Date().toISOString();
  const currentPriceEur = product.cm_lowest;
  const differenceEur = currentPriceEur == null ? null : Number((marketOffer.priceEur - currentPriceEur).toFixed(2));
  return {
    currentPriceEur, observedPriceEur: marketOffer.priceEur, differenceEur,
    differencePercent: currentPriceEur != null && currentPriceEur > 0 ? Number(((marketOffer.priceEur - currentPriceEur) / currentPriceEur * 100).toFixed(1)) : null,
    offerCount: sellerOffers.length, sourceUrl: scrape.sourceUrl, provider: scrape.provider, observedAt,
    sealedMarkets: [
      { label: "Market", savedPriceEur: currentPriceEur, offer: marketOffer },
      { label: "EU Market", savedPriceEur: product.cm_lowest_eu, offer: euOffer },
    ],
    sealedOffers: sellerOffers.slice(0, 10),
    scrapedName: title, scrapedSetName: null, scrapedCardNumber: null,
    token: createAdminCardMarketPriceCheckToken({ v: 1, cardId: `sealed:${id}`, priceEur: marketOffer.priceEur, offerCount: sellerOffers.length, sourceUrl: scrape.sourceUrl, provider: scrape.provider, observedAt, sealedEuPriceEur: euOffer?.priceEur ?? null }),
  };
}
...
export async function confirmAdminSealedCardMarketPriceCheck(id: string, token: string, decision: "changed" | "unchanged") {
  ...
  await db.$transaction(async (tx) => {
    ...
    if (euPrice != null && (!Number.isFinite(euPrice) || euPrice < payload.priceEur)) {
      throw new AdminCardMarketPriceCheckError("Invalid EU market observation. Run the check again.");
    }
    await tx.sealedProduct.update({ where: { id }, data: { cm_lowest: payload.priceEur, ...(euPrice != null ? { cm_lowest_eu: euPrice } : {}) } });
    await tx.sealedPriceSnapshot.create({ data: { ...buildSealedCardMarketSnapshotData({ productId: id, episodeId: product.episode_id, priceEur: payload.priceEur, observedAt: new Date(payload.observedAt) }), cm_lowest_eu: euPrice ?? null } });
  });
}
