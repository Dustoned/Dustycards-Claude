...
import { Check, Link2, Loader2, Maximize2, RefreshCw, Undo2, Unlink } from "lucide-react";
...
type ReviewHistory = { id: string; updatedAt: string; decision: string; source: ReviewCard; target: ReviewCard };
...
export default function ReprintReviewSection() {
  ...
  async function undo(review: ReviewHistory) {
    setBusyKey(`undo:${review.id}`);
    setError(null);
    setNotice(null);
    try {
      const response = await fetch("/api/admin/reprint-review", {
        method: "DELETE", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ id: review.id, updatedAt: review.updatedAt }),
      });
      const payload = await response.json();
      if (!response.ok) throw new Error(payload.error ?? "Could not undo review");
      await load(false);
      setNotice(`${review.source.name}: returned to the top of the review queue.`);
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : "Could not undo review");
    ...
    <section className="rounded-3xl border border-violet-300/12 bg-violet-500/[0.035] p-4 sm:p-5">
      <div className="flex flex-col items-start justify-between gap-3 sm:flex-row">
        ...
        <div className="flex shrink-0 flex-wrap items-center gap-2">
          ...
          <button type="button" onClick={() => void undo(history[0])} disabled={loading || busyKey !== null || !history.length} className="flex min-h-11 items-center gap-1.5 rounded-xl border border-violet-300/20 px-3 text-xs font-bold text-violet-100 disabled:opacity-40">
            <Undo2 className="h-4 w-4" /> Undo last
          </button>
          <button type="button" onClick={() => void load()} disabled={loading || busyKey !== null} className="flex h-9 items-center gap-1.5 rounded-xl border border-white/9 px-2.5 text-[10px] font-black text-white/58">
      ...
      {history.length ? <details className="mt-3 rounded-xl border border-white/10 p-3 text-xs">
              ...
              <article key={key} className="rounded-2xl border border-white/8 bg-black/16 p-3 sm:p-5">
                ...
                <div className="mt-4 border-t border-white/7 pt-4">
                  <p className="text-center text-[9px] font-bold uppercase tracking-[0.09em] text-white/30">
                    {item.matchMethod === "returned-for-review" ? "Returned for review · choose again" : `Visual candidate · ${Math.round(item.imageSimilarity * 100)}% colour and artwork agreement`}
                  </p>
