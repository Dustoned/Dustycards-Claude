...
export default async function AdviceLearningPage({searchParams}:{searchParams:Promise<{context?:string;origin?:string;horizon?:string;label?:string;result?:string;sample?:string;page?:string}>}) {
  ...
  const label=["buy","hold","sell"].includes(params.label??"")?params.label!:"all";
  ...
  const filtered={...where,observation:{...observation,...(label!=="all"?{label:{in:families[label as keyof typeof families]}}:{})},...(result==="correct"||result==="missed"?{status:"complete",correct:result==="correct"}:result!=="all"?{status:result}:{})};
  ...
  return <main className="mx-auto max-w-7xl space-y-6 px-4 py-6 text-white/85 sm:px-6 sm:py-10">
    ...
    <section><div className="mb-4 flex flex-wrap items-center justify-between gap-3"><h2 className="text-lg font-semibold">Saved advice · {count.toLocaleString("en-GB")}</h2>{choice("Advice labels",["all","buy","hold","sell"].map(value=>({label:value==="all"?"All labels":value.toUpperCase(),value})),"label",label)}</div>
    ...
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{rows.map(row=>{const entry=row.observation;let evidence:{reasons?:string[];warnings?:string[];metrics?:{history_points?:number}}={};try{evidence=JSON.parse(entry.evidence_json);}catch{/* Legacy malformed evidence. */}return <article key={row.id} className={panel}><div className="flex flex-wrap items-center justify-between gap-2"><span className={`rounded-lg px-3 py-1 text-xs font-bold ${colors[adviceFamily(entry.label)]}`}>{entry.label.replaceAll("_"," ").toUpperCase()}</span><span className="text-xs text-white/55">{row.status==="complete"?(row.correct?"Correct":"Missed"):row.status==="pending"?"Watching":"Needs data"}</span></div><h3 className="mt-4 font-bold">{entry.card_name}</h3><p className="mt-1 text-xs text-white/45">{entry.confidence} confidence · score {Math.round(entry.score)} · {entry.grade_label??"Raw EN NM"}</p><dl className="mt-4 grid grid-cols-2 gap-3 text-sm"><div><dt className="text-xs text-white/40">Entry · {entry.currency}</dt><dd>{entry.entry_price.toFixed(2)}</dd></div><div><dt className="text-xs text-white/40">After {horizon} days</dt><dd>{row.end_price==null?"—":row.end_price.toFixed(2)} · {pct(row.return_pct)}</dd></div></dl><p className="mt-4 text-xs leading-5 text-white/50">{evidence.reasons?.slice(0,3).join(" · ")||"No recorded reasons"}</p>{evidence.warnings?.length?<p className="mt-2 text-xs leading-5 text-amber-200/70">{evidence.warnings.slice(0,2).join(" · ")}</p>:null}<p className="mt-4 border-t border-white/10 pt-3 text-xs leading-5 text-white/40">{origin==="replay"?"Replayed":"Recorded"} {day(entry.observed_at)} · Due {day(row.due_at)}<br/>{entry.source} · {row.observed_days} observed days<br/>{entry.model_version} · {evidence.metrics?.history_points??0} history points at entry</p></article>;})}</div>
