...
export default async function LearningPage({ searchParams }: { searchParams: Promise<{ horizon?: string; result?: string; page?: string; outcome?: string; day?: string }> }) {
  ...
  const dayRange = learningDayRange(params.day);
  const dayFilter = dayRange ? { evaluated_at: dayRange, status: "complete", meaningful_direction_hit: { not: null } } : {};
  const horizon = params.horizon === "all" || (dayRange && !params.horizon) ? 0 : [30, 90, 180].includes(Number(params.horizon)) ? Number(params.horizon) : focused?.horizon_days ?? 30;
  ...
  const where = { ...dayFilter, ...(horizon ? { horizon_days: horizon } : {}), entry_observation: visibility };
  ...
  const [groups, rows, count] = await Promise.all([
    db.externalSignalOutcome.groupBy({ by: ["horizon_days", "status", "meaningful_direction_hit"], where: { ...dayFilter, entry_observation: visibility }, _count: { _all: true } }),
  ...
  const stats = summarizeLearning(groups.filter(row => !horizon || row.horizon_days === horizon));
  const href = (h = horizon, r = result, p = 1) => `${base}?horizon=${h || "all"}&result=${r}&page=${p}${dayRange ? `&day=${params.day}` : ""}`;
  ...
  return <main className="mx-auto max-w-7xl space-y-6 px-4 py-6 text-white/85 sm:px-6 sm:py-10">
    ...
    <nav aria-label="Prediction horizon" className="grid grid-cols-2 gap-2 sm:grid-cols-4">{[0,30,90,180].map(h => <Link prefetch={false} key={h} href={href(h)} aria-current={horizon === h ? "page" : undefined} className={`rounded-xl border px-4 py-3 text-center text-sm font-semibold ${horizon === h ? "border-violet-400/50 bg-violet-500/20 text-violet-100" : "border-white/10 text-white/50"}`}>{h ? `${h} days` : "All periods"}</Link>)}</nav>
