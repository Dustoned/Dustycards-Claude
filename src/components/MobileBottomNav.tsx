  ...
  return (
    <>
      {moreOpen && (
        ...
        >
          <div
            className="absolute inset-x-2 bottom-[calc(4.15rem+env(safe-area-inset-bottom))] max-h-[min(68dvh,34rem)] overflow-y-auto rounded-[22px] border border-white/10 bg-[#070708]/97 p-2 shadow-[0_28px_90px_rgba(0,0,0,0.68)] [scrollbar-width:none] backdrop-blur-xl [&::-webkit-scrollbar]:hidden"
            onClick={(event) => event.stopPropagation()}
          >
            <div className="mb-1.5 flex items-center justify-between gap-3 px-1">
              <div className="min-w-0">
                <p className="text-[11px] font-black uppercase tracking-[0.14em] text-white/34">Menu</p>
              </div>
              <button
                type="button"
                onClick={() => {
                ...
                className="inline-flex h-8 w-8 items-center justify-center rounded-full border border-white/10 bg-white/[0.055] text-white/70"
            ...
            {summary ? (
              <div className="mb-2 rounded-[18px] border border-white/10 bg-gradient-to-b from-white/[0.055] to-white/[0.024] p-2 shadow-[0_16px_36px_rgba(0,0,0,0.2)]">
                <div className="flex items-center gap-2">
                  <Link
                    href="/account"
                    prefetch={false}
                    onClick={() => setMoreOpen(false)}
                    className="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl border border-violet-300/18 bg-violet-500/18 text-xs font-black text-violet-100 shadow-[0_0_18px_rgba(139,92,246,0.18)]"
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-0.5 flex min-w-0 items-center gap-1.5 text-[10px] font-semibold text-white/38">
                      <Mail className="h-3 w-3 shrink-0" />
                      <span className="truncate">{summary.email}</span>
                    </div>
                    <p className="mt-1 truncate text-[10px] font-bold text-white/40">
                ...
                <div className="mt-2 grid grid-cols-[1fr_auto] gap-1">
                  <Link
                    ...
                    prefetch={false}
                    onClick={() => setMoreOpen(false)}
                    className="inline-flex min-h-8 items-center justify-center rounded-xl border border-white/10 bg-white/[0.055] px-3 text-[11px] font-black text-white/78"
                  >
                    Profile
                  </Link>
                  <button
                    ...
                    onClick={logout}
                    disabled={loggingOut}
                    className="inline-flex min-h-8 items-center justify-center gap-1.5 rounded-xl border border-rose-300/18 bg-rose-500/[0.075] px-3 text-[11px] font-black text-rose-100 disabled:cursor-wait disabled:opacity-60"
                    aria-label="Log out"
                  >
            ...
            <div className="grid gap-1.5">
              {mobileSections.map((section) => (
                <section key={section.label} className="rounded-2xl border border-white/8 bg-white/[0.026] p-1.5">
                  <p className="px-2 pb-1 text-[9px] font-black uppercase tracking-[0.14em] text-white/32">
                    {section.label}
                  </p>
                    ...
                    {section.items.map((item) => {
                          ...
                          >
                            <button
                              ...
                              aria-expanded={expansionsOpen}
                                  className={`flex min-h-9 min-w-0 items-center gap-2 rounded-xl border px-2.5 text-left text-[11px] font-bold transition-colors ${
                                    active
                            ...
                            {expansionsOpen ? (
                              <div className="grid grid-cols-2 gap-1">
                                <Link
                                  ...
                                  className={`flex min-h-9 items-center justify-center rounded-lg border px-2 text-[11px] font-bold transition-colors ${
                      ...
                      return (
                        <Link
                          ...
                          className={`flex min-h-9 min-w-0 items-center gap-2 rounded-xl border px-2.5 text-[11px] font-bold transition-colors ${
                            active
                              ? "border-white/16 bg-white/[0.08] text-white"
                              : "border-transparent text-white/62 hover:border-white/10 hover:bg-white/[0.055] hover:text-white"
                          }`}
