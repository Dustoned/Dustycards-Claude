...
export function sealedTileRootClass(): string {
  return "group flex cursor-pointer flex-col gap-1.5 text-left outline-none";
}
...
export function sealedTileTitleClass(cardSize: CardSize): string {
  if (cardSize === "large") {
    return "line-clamp-2 text-[18px] font-semibold leading-snug text-gray-900 dark:text-white";
  }

  if (cardSize === "medium") {
    return "line-clamp-2 text-[15px] font-semibold leading-snug text-gray-900 dark:text-white";
  }

  return "line-clamp-2 text-[13px] font-semibold leading-snug text-gray-900 dark:text-white";
}
...
export function sealedTileBubbleWrapClass(cardSize: CardSize): string {
  if (cardSize === "large") {
    return "mt-3 flex flex-wrap items-center gap-2";
  }

  if (cardSize === "medium") {
    return "mt-2.5 flex flex-wrap items-center gap-1.5";
  }
...
export function sealedTileBubbleClass(tone: SealedTileTone = "neutral"): string {
  const base =
    "inline-flex min-h-[var(--ui-chip-min-height)] shrink-0 items-center gap-[var(--ui-chip-gap)] whitespace-nowrap rounded-full border px-[var(--ui-chip-x)] py-[var(--ui-chip-y)] text-[length:var(--ui-chip-font-size)] font-medium leading-none shadow-sm shadow-black/5 dark:shadow-black/20";
...
export function sealedTileBubbleLabelClass(): string {
  return "text-[length:var(--ui-chip-count-font-size)] font-semibold uppercase tracking-[0.12em] opacity-70";
}
